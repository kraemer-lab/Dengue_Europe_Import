...
df_unobs = df_unobs.sort_values('term', ascending=True)
...
ax[0].axvspan(13, len(df_term), ymin=0, ymax=1, alpha=0.2, color='grey', label='Unobserved period')
...
for i in range(len(df_r_country)):
    ax[1].text(np.arange(len(df_r_country))[i]-0.1, df_r_country.y_u.values[i]+50, str(sim_rep.SI.values.round(2)[i]), size=18)
ax[1].legend(prop={'size': 20})
ax[1].text(-1.8, 1000, "D", size=35)
ax[1].grid(alpha=0.3)
...
for i in range(len(df_country)):
    ax[2].text(np.arange(len(df_country))[i], df_country.y_u.values[i]+50, str(sim_exp.SI.values.round(1)[i]), size=18)
ax[2].legend(prop={'size': 20})
ax[2].text(-1.8, 500, "E", size=35)
ax[2].grid(alpha=0.3)
...
ax_top.set_title("Zika importations aggregated by quarter")
ax_top.spines[['right', 'top']].set_visible(False)
ax_top.set_ylim(ymin=0, ymax=500)
...
ax[1].axis("off")
ax[1].set_title("Average Zika Importations", size=16)
