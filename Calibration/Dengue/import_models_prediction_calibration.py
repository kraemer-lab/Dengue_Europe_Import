...
ax[0].set_ylim(ymin=0, ymax=2000)
...
for i in range(len(df_r_country)):
    ax[1].text(np.arange(len(df_r_country))[i]-0.1, df_r_country.y_u.values[i]+1000, str(sim_rep.SI.values.round(2)[i]), size=18)
ax[1].legend(prop={'size': 20})
ax[1].text(-2, 7000, "D", size=35)
ax[1].grid(alpha=0.3)
...
for i in range(len(df_country)):
    ax[2].text(np.arange(len(df_country))[i]-0.1, df_country.y_u.values[i]+550, str(sim_exp.SI.values.round(2)[i]), size=18)
ax[2].legend(prop={'size': 20})
ax[2].text(-2.3, 4500, "E", size=35)
ax[2].grid(alpha=0.3)
...
ax_top.set_title("Dengue importations aggregated by quarter")
ax_top.spines[['right', 'top']].set_visible(False)
ax_top.set_ylim(ymin=0, ymax=2000)
