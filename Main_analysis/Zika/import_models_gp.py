...
df0 = pd.read_csv("./data/zika_clean_data_travel.csv")
...
with pm.Model(coords=coords) as mod:
    ...
    epsi_z = pm.Normal("epsi_z", 0, 1, dims="dyad")
    epsi_s = pm.HalfNormal("epsi_s", 1)
    epsi = pm.Deterministic("epsi", epsi_l + epsi_s*epsi_z, dims="dyad")
...
dfu = df0[df0.year=='2019'][['travellers_ee', 'reporting_country']].drop_duplicates(subset="reporting_country")
...
with mod:
    ...
    delta_u_z = pm.Normal("delta_u_z", 0, 1, dims="dyad_u")
    delta_u = pm.Deterministic("delta_u", delta_l + delta_s*delta_u_z, dims="dyad_u")
    
    epsi_u_z = pm.Normal("epsi_u_z", 0, 1, dims="dyad_u")
    epsi_u = pm.Deterministic("epsi_u", epsi_l + epsi_s*epsi_u_z, dims="dyad_u")

    lam_u = pm.Deterministic("lam_u", tau_u[t_u_idx] + delta_u[d_u_idx] + epsi_u[e_u_idx]*xun)
...
with mod:
    pred_u = pm.sample_posterior_predictive(idata, var_names=["y_u", 'lam_u'], predictions=True, random_seed=33)
...
lam = np.concatenate([az.extract(pred0.posterior_predictive)['lam'].values, az.extract(pred_u.predictions)['lam_u'].values])
...
ax[0].axvspan(24+0.1, len(df_term)-0.1, ymin=0, ymax=1, alpha=0.2, color='grey', label='Unobserved period')
...
ax[0].set_title("Zika importations aggregated by quarter")
...
ax[1].set_title("Zika importations aggregated by reporting country")
...
ax[2].set_title("Zika importations aggregated by exporting country")
ax[2].spines[['right', 'top']].set_visible(False)
ax[2].set_ylim(ymin=0, ymax=500)
...
fig.suptitle("Predicted Zika importations to Europe", y=0.95, size=18)
plt.tight_layout()
...
for i in range(len(rph)):
    ...
    ax.grid(alpha=0.5)
ax.set_title("Zika 2015-2025", pad=20, size=30)
